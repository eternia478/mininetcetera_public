...
class PlainSwitch ( Switch ):
    def attach( self, intf ):
        print self, "attach", intf

    def detach( self, intf ):
        print self, "detach", intf
...
class POXNGSwitch( Switch ):
    "Switch to run a POX application."

    def __init__( self, name,
                  address="127.0.0.1", port=6644, ports="", dpid=None,
                  extra="", listenPort=None, **params):
        """Init.
        ...
        assert self.name.endswith(self.real_dpid)
        ...
        #TODO: use self.extra and self.max_retry_delay
        ...
        #      print "CALL",name
        #      return f(*args, **kw)
        #  setattr(self, attr, makeFoo(attr, getattr(self, attr)))

    @property
    def real_dpid (self):
        return str(int(self.dpid, 16))

    def _build_cmd_args( self ):
        "Build command-line argument of POX."

        cmd = self.poxCoreDir + "/pox.py"
        pathCheck( cmd )

        args = []
        args.append(cmd)

        args.append("--no-openflow")
        ...
        args.append("--port=%s" % self.controller_port)
        args.append("--dpid=%s" % self.real_dpid)
    ...
    def attach( self, intf ):
        "Connect a data port"
        self.cmd( 'ifconfig', intf, '0.0.0.0 up' )
        #intf_name = str(intf)
        #

...
def cleanup():
    ...
    # Remove unkilled pox processes.
    # NOTE: This may accidentally kill remote controllers as well.
    info("*** Removing excess pox processes\n")
    sh( 'kill `pgrep -f pox.py` 2> /dev/null')
    time.sleep(1)
    sh( 'kill -9 `pgrep -f pox.py` 2> /dev/null')
